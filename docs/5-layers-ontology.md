...
This document defines the official 5-Layer Graph Ontology architecture for CodeExplorer. The ontology is designed to decouple physical file layouts, syntactic code structures, runtime semantic interfaces, and cross-project integration links into separate queryable layers connected by uniform reference pointers.
...
```mermaid
graph TD
    Workspace[Workspace] -->|CONTAINS| Project[Project]

    subgraph Layer1 [Layer 1: Physical Topology]
        FilesStructure[FilesStructure] -->|CONTAINS| Folder[Folder]
    ...
    subgraph Layer4 [Layer 4: Semantic Runtime]
        ...
        SemanticStructure -->|CONTAINS| Package[Package]
    end
...
At the absolute top of the hierarchy is the `Workspace`. This does not belong to any layer; it is the root/umbrella container that holds all projects, and acts as the boundaries scope.
...
Tracks the exact directory layout of the workspace on disk. It remains strictly decoupled from syntactic AST models, ensuring physical layout queries (e.g., file trees) do not require traversing source code symbol structures.

### Nodes

#### 1. **`FilesStructure`**
*   **Description:** A structural grouping node representing the physical folder and file tree of a project.
*   **Properties:**
...
## 📂 Layer 2: Project Boundary (Module Scopes)

Defines the logical package boundaries or compilation scopes (e.g., C# `.csproj` file, Go `go.mod` directory, TypeScript `package.json` directory).
...
This layer represents the declarations found inside AST parser visitors (Tree-sitter and ScriptDom). Nodes in this layer represent source code definitions and syntax outlines, completely isolated from runtime infrastructure.
...
#### 7. **`Package`**
*   **Description:** Third-party package dependencies (e.g., NuGet, NPM, Go modules).
*   **Properties:**
...
| **Layer 2** | **`FilesStructure`** | `{workspaceId}:project:{relativeProjectDir}:files_structure` | `1:project:Core/:files_structure` |
| **Layer 2** | **`SyntaxStructure`** | `{workspaceId}:project:{relativeProjectDir}:syntax_structure` | `1:project:Core/:syntax_structure` |
| **Layer 2** | **`SemanticStructure`** | `{workspaceId}:project:{relativeProjectDir}:semantic_structure` | `1:project:Core/:semantic_structure` |
...
| **Layer 4** | **`Package`** | `{workspaceId}:package:{packageId}` | `1:package:nestjs-common` |

---

## 💡 Benefits for Surgical Pruning & Indexing

The 5-Layer model directly simplifies incremental file parsing:
